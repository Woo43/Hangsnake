...
while playercount != 1 and playercount != 2:
	try:
		...
		playercount = int(input("Are you playing with 1 or 2 players? "))
if playercount == 2:
	allowedcharlist = open("allowedletters.txt").readlines()
	allowedchars = allowedcharlist[0].split(",")
	secretword = input("Select a secret word. ")
	secondlist = list(secretword.upper())
	if set(secondlist).issubset(set(allowedchars)) == False:
		quit()
	try:
...
letters = []
if set(spacelist).issubset(set(secretlist)) == True:
    letters.append(" ")
    letters_correct += 1
while oofcount < maxguesses and winner == False:
	...
	if guess in letters:
		...
		if guess in secretlist:
			...
			oofcount = oofcount + 1
			print("You have " + str(maxguesses - oofcount) +
